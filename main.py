...
class MainMenu(Menu):
    def __init__(self,parent,*args,**kwargs):
        ...
        self.edit.add_command(label="Undo",accelerator="Ctrl+Z",command=self.parent.undofunc)
        self.edit.add_command(label="Redo",accelerator="Ctrl+Y",command=self.parent.redofunc)
        ...
        self.edit.add_command(label='Delete',accelerator="Del")
        self.edit.add_separator()
        self.edit.add_command(label='Select All',accelerator="Ctrl+A",command=self.parent.event_generate("<<Select All>>"))
        self.edit.add_command(label='Clear All',accelerator="Ctrl+Alt+C",command=lambda :self.parent.texteditor.delete(1.0,END))
        self.edit.add_command(label='Date/Time',accelerator="F5",command=self.parent.datetimefunc)

        #########################Find/Replace Menu########################
        self.findrep=Menu(self,tearoff=0)
        self.findrep.add_command(label='Find/Replace',accelerator="Ctrl+F",command=self.parent.find)

        ###########################View Menu############################
        global showStatusbar
        global showToolbar
        self.view=Menu(self,tearoff=0)
        self.view.add_checkbutton(onvalue=1, offvalue=0,label="Tool Bar",variable=showToolbar,
        ...
        self.add_cascade(label='Find/Replace', menu=self.findrep)
...
class MainApplication(Frame):
    ...
    def aboutMessage(self,*args):
        messagebox.showinfo("About","This is the text editor made using Python Tkinter.")
    ...
    def saveAsFile(self,*args):
        try:
            ...
            url.write(content)
            url.close()
            self.parent.title("textEditor - Now Editing "+str(url.split('/')[-1]))
        except:
            return
    ...
    def undofunc(self,*args):
        global texteditor
        try:
            self.texteditor.edit_undo()
        except:
            pass


    def redofunc(self,*args):
        global texteditor
        try:
            self.texteditor.edit_redo()
        except:
            pass
    ...
    def changed(self,*args):
        ...
        if flag:
            ...
            lines=int(self.texteditor.index('end').split('.')[0]) - 1
            self.statusbar.config(text="Characters: " +str(letters)+ "     | Words: " +str(words)+"     | Lines: "+str(lines))

        self.texteditor.edit_modified(False)


    def datetimefunc(self,*args):
        global texteditor
        self.texteditor.insert(END,str(datetime.datetime.now()))
